      ...
      <div className="mx-auto flex h-18 max-w-7xl items-center gap-4 px-4">
          ...
          <SheetContent side="left">
            ...
            <nav className="grid gap-2">
              {navItems.map(([key, href]) => (
                <Link
                  className="rounded-md px-3 py-3 font-semibold hover:bg-accent"
                  href={`/${locale}/${href}`}
                  key={key}
                >
                  {dictionary.nav[key]}
        ...
        <Link href={`/${locale}`} className="flex items-center gap-3">
          <Image
            ...
            priority
          />
          <span className="hidden font-black text-xl sm:inline">BOOM</span>
        </Link>
        <nav className="hidden flex-1 items-center justify-center gap-1 md:flex">
          {navItems.map(([key, href]) => (
            <Button asChild variant="ghost" key={key}>
              <Link href={`/${locale}/${href}`}>{dictionary.nav[key]}</Link>
            </Button>
          ))}
        </nav>
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button asChild variant="ghost" size="icon" aria-label={dictionary.nav.favorites}>
